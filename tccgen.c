...
static Sym* parse_transparent_alias_decl(AttributeDef *ad, CType *t)
{
  ...
  if (alias < TOK_UIDENT)
    ...
    tcc_warning("got identifier");

  tcc_warning("Got alias '%s'", get_tok_str(alias, NULL));
  ...
  tcc_warning("Got alias target '%s'", get_tok_str(target, NULL));
  ...
  *t = target_sym->type;
  ...
  // memcpy(s, target_sym, sizeof(Sym));
  ad->alias_target = target;
  // ad->asm_label = tok_alloc(target_str, strlen(target_str))->tok;
  // sym_to_attr(ad, target_sym);
